 ...
 */
class ProgrisaaS_ImageBox_Carousel extends Widget_Base{
	...
	public function get_title() {
		return __( 'OT Image Box Carousel', 'progrisaas' );
	}
	...
	protected function register_controls() {
		...
		$this->start_controls_section(
			'content_section',
			[
				'label' => __( 'Image Box Carousel', 'progrisaas' ),
			]
		);
		...
		$repeater->add_control(
			'title',
			[
				'label' => __( 'Title/Name', 'progrisaas' ),
		...
		$repeater->add_control(
			'vlink',
			[
				'label' => __( 'Link Video Popup', 'progrisaas' ),
				'type' => Controls_Manager::TEXT,
		...
		$repeater->add_control(
			'link',
			[
				'label' => __( 'Link to Detail', 'progrisaas' ),
		...
		$this->add_control(
			'add_settings',
			[
				'label' => __( 'Additional settings', 'progrisaas' ),
		...
		$this->add_control(
			'btn_video_color',
			[
				'label' => __( 'Button Video Color', 'progrisaas' ),
				'type' => Controls_Manager::COLOR,
				'default' => '',
				'selectors' => [
					'{{WRAPPER}} .ot-image-box__video' => 'border-color: {{VALUE}};',
					'{{WRAPPER}} .ot-image-box__video i' => 'color: {{VALUE}};',
				]
			]
		);
	...
	protected function render() {
		...
		$gaps   = isset( $settings['slide_spacing']['size'] ) && is_numeric( $settings['slide_spacing']['size'] ) ? $settings['slide_spacing']['size'] : 70;
		$tgaps  = isset( $settings['slide_spacing_tablet']['size'] ) && is_numeric( $settings['slide_spacing_tablet']['size'] ) ? $settings['slide_spacing_tablet']['size'] : $gaps;
		$mgaps  = isset( $settings['slide_spacing_mobile']['size'] ) && is_numeric( $settings['slide_spacing_mobile']['size'] ) ? $settings['slide_spacing_mobile']['size'] : $tgaps;
			...
			?> 
				...
				<div class="ot-image-box">
					...
					<div class="ot-image-box__content">
		        		...
		        		<?php } ?>
						<h5 class="image-box-title"><?php echo $title; ?></h5>
						<?php if( $item['desc'] ) { echo '<div class="image-box-des">' .$item['desc']. '</div>'; } ?>
					</div>
				</div>
				<?php } ?>
...
// After the ProgrisaaS_ImageBox_Carousel class is defined, I must register the new widget class with Elementor:
Plugin::instance()->widgets_manager->register_widget_type( new ProgrisaaS_ImageBox_Carousel() );